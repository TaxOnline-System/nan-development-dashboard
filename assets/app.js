...
function renderTable(){
  ...
  $('tableBody').innerHTML=state.filtered.map(p=>{
    const hasSdgs=Array.isArray(p.sdgs)&&p.sdgs.length>0;
    const tableSdgs=hasSdgs
      ? p.sdgs.map(x=>`<span class="pill">SDG ${x}</span>`).join('')
      : '<span class="no-data-text">ไม่พบข้อมูล</span>';

    const rawAlignment=String(p.alignment||'').trim();
    const isMissingAlignment=
      !rawAlignment ||
      rawAlignment==='ไม่ระบุ' ||
      rawAlignment==='ไม่พบข้อมูล' ||
      rawAlignment.includes('อยู่ระหว่างการวิเคราะห์ข้อมูล');
    const tableAlignment=isMissingAlignment?'ไม่พบข้อมูล':rawAlignment;
