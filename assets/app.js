...
window.detail=function(id){const p=state.projects.find(x=>x.id===id);if(!p)return;const score=scoreProject(p);$('detailCategory').textContent=`${p.formLabel} • ${p.category} • ${money(p.budget)} • ${pct(p.budgetPercent)} ของงบรวม`;$('detailTitle').textContent=p.projectName;$('detailBody').innerHTML=`<div class="detail-grid"><div class="assessment-box full"><div class="score-wrap"><div class="score">${score}<small>Alignment Score / 100</small></div><div><div class="progress"><span style="width:${score}%"></span></div><p class="muted">คะแนนจากความครบถ้วนของประเด็นและแนวทางพัฒนา แผนงานหลัก ตัวชี้วัด Output Outcome SDGs ค่าเป้าหมาย และผู้ได้รับประโยชน์</p></div></div></div><div class="detail-card"><h4>ข้อมูลโครงการ</h4><p>ประเภท: ${escapeHtml(p.formLabel)}\nหน่วยงาน: ${escapeHtml(p.agency)}\nกลุ่ม: ${escapeHtml(p.category)}\nงบประมาณ: ${money(p.budget)} (${pct(p.budgetPercent)})</p></div><div class="detail-card"><h4>ผลประเมินและ SDGs</h4><p>ระดับ: ${escapeHtml(p.alignment)}\nSDGs: ${(p.sdgs||[]).map(s=>'SDG '+s).join(', ')||'ไม่ระบุ'}\nKeyword: ${escapeHtml(p.sdgKeyword)}</p></div><div class="detail-card full criteria">
  <h4>ความเชื่อมโยงกับแผนพัฒนาจังหวัด</h4>

  <div class="plan-group">

    <div class="plan-box issue">
      <div class="plan-box-title">ประเด็นการพัฒนา</div>
      <div class="plan-box-content">
        ${escapeHtml(p.developmentIssue || 'ไม่พบข้อมูล')}
      </div>
    </div>

    <div class="plan-box approach">
      <div class="plan-box-title">แนวทางการพัฒนา</div>
      <div class="plan-box-content">
        ${escapeHtml(p.developmentApproach || 'ไม่พบข้อมูล')}
      </div>
    </div>

    <div class="plan-box main-plan">
      <div class="plan-box-title">แผนงานหลัก</div>
      <div class="plan-box-content">
        ${escapeHtml(p.plan || 'ไม่พบข้อมูล')}
      </div>
    </div>

    <div class="plan-box indicator">
      <div class="plan-box-title">ตัวชี้วัด</div>
      <div class="plan-box-content ${
        !p.indicator || p.indicator === 'ไม่พบข้อมูล'
          ? 'no-data'
          : ''
      }">
        ${escapeHtml(p.indicator || 'ไม่พบข้อมูล')}
      </div>
    </div>

    <div class="plan-box criteria-box">
      <div class="plan-box-title">เกณฑ์ประเมิน</div>
      <div class="plan-box-content ${
        !p.criteria || p.criteria === 'ไม่พบข้อมูล'
          ? 'no-data'
          : ''
      }">
        ${escapeHtml(p.criteria || 'ไม่พบข้อมูล')}
      </div>
    </div>

  </div>
</div><div class="detail-card"><h4>Output ที่ระบุ</h4><p>${escapeHtml(p.output)}</p></div><div class="detail-card"><h4>Outcome ที่ระบุ</h4><p>${escapeHtml(p.outcome)}</p></div><div class="detail-card full"><h4>เหตุผลการประเมิน</h4><p>${escapeHtml(p.assessment)}</p></div><div class="detail-card full recommend"><h4>ข้อเสนอแนะหลัก</h4><p>${escapeHtml(p.suggestion)}</p></div></div>`;$('detailDialog').showModal();};
