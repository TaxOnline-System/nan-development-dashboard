...
const FIELD_ALIASES = {
  ...
  indicator:['ตัวชี้วัดของแผน','ตัวชี้วัดที่สนับสนุน','ตัวชี้วัดและค่าเป้าหมาย','ตัวชี้วัด'],
  ...
  sdgText:['sdgsที่สอดคล้อง','sdgs','sdg'], sdgKeyword:['คำอธิบายsdgskeyword','keywordsdg','คำอธิบายsdgs','keyword'],
...
function updateHeading(){
  ...
  $('pageTitle').textContent=title;$('pageSubtitle').textContent=`${sub} พร้อมรายละเอียดสิ่งที่ต้องปรับแก้ให้สอดคล้องกับแผนงานหลัก ตัวชี้วัด และ SDGs`;
}
function apply(){
...
function renderTable(){$('resultCount').textContent=`แสดง ${THB.format(state.filtered.length)} รายการ`;$('tableBody').innerHTML=state.filtered.map(p=>`<tr><td><b>${escapeHtml(p.formLabel)}</b></td><td>${escapeHtml(p.category)}</td><td class="project-cell"><strong>${escapeHtml(p.projectName)}</strong><div class="muted">${escapeHtml(p.agency)}</div></td><td class="plan-cell"><span><b>แผนงาน:</b> ${escapeHtml(p.plan)}</span><span><b>ตัวชี้วัด:</b> ${escapeHtml(p.indicator)}</span></td><td class="money">${money(p.budget)}</td><td class="pct">${pct(p.budgetPercent)}</td><td>${sdgPills(p.sdgs)}</td><td><span class="level ${levelClass(p.alignment)}">${escapeHtml(p.alignment)}</span></td><td><button class="detail-open" onclick="detail('${escapeHtml(p.id)}')">เปิดรายงาน</button></td></tr>`).join('')||'<tr><td colspan="9" class="empty">ไม่พบข้อมูล</td></tr>';}
window.detail=function(id){const p=state.projects.find(x=>x.id===id);if(!p)return;const score=scoreProject(p),checks=checklist(p);$('detailCategory').textContent=`${p.formLabel} • ${p.category} • ${money(p.budget)} • ${pct(p.budgetPercent)} ของงบรวม`;$('detailTitle').textContent=p.projectName;$('detailBody').innerHTML=`<div class="detail-grid"><div class="assessment-box full"><div class="score-wrap"><div class="score">${score}<small>Alignment Score / 100</small></div><div><div class="progress"><span style="width:${score}%"></span></div><p class="muted">คะแนนจากความครบถ้วนของประเด็นและแนวทางพัฒนา แผนงานหลัก ตัวชี้วัด Output Outcome SDGs ค่าเป้าหมาย และผู้ได้รับประโยชน์</p></div></div></div><div class="detail-card"><h4>ข้อมูลโครงการ</h4><p>ประเภท: ${escapeHtml(p.formLabel)}\nหน่วยงาน: ${escapeHtml(p.agency)}\nกลุ่ม: ${escapeHtml(p.category)}\nงบประมาณ: ${money(p.budget)} (${pct(p.budgetPercent)})</p></div><div class="detail-card"><h4>ผลประเมินและ SDGs</h4><p>ระดับ: ${escapeHtml(p.alignment)}\nSDGs: ${(p.sdgs||[]).map(s=>'SDG '+s).join(', ')||'ไม่ระบุ'}\nKeyword: ${escapeHtml(p.sdgKeyword)}</p></div><div class="detail-card full criteria"><h4>ความเชื่อมโยงกับแผนพัฒนาจังหวัด</h4><p>ประเด็นการพัฒนา: ${escapeHtml(p.developmentIssue)}\nแนวทางการพัฒนา: ${escapeHtml(p.developmentApproach)}\nแผนงานหลัก: ${escapeHtml(p.plan)}\nตัวชี้วัด: ${escapeHtml(p.indicator)}\n\nเกณฑ์ประเมิน: ${escapeHtml(p.criteria)}</p></div><div class="detail-card full"><h4>Checklist พร้อมข้อมูลที่ต้องนำไปปรับแก้</h4><div class="project-checklist">${checks.map(checkHtml).join('')}</div></div><div class="detail-card"><h4>Output ที่ระบุ</h4><p>${escapeHtml(p.output)}</p></div><div class="detail-card"><h4>Outcome ที่ระบุ</h4><p>${escapeHtml(p.outcome)}</p></div><div class="detail-card full"><h4>เหตุผลการประเมิน</h4><p>${escapeHtml(p.assessment)}</p></div><div class="detail-card full recommend"><h4>ข้อเสนอแนะหลัก</h4><p>${escapeHtml(p.suggestion)}</p><div class="copy-sample"><b>ตัวอย่าง Outcome สำหรับนำไปปรับใช้</b><code>${escapeHtml(suggestedOutcome(p))}</code></div></div></div>`;$('detailDialog').showModal();};
